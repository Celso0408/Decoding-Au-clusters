"""
Rigorous Verification of UMAP Core Reduction (DimRedPy vs umap-learn)
...
data = np.loadtxt(data_path, max_rows=1000) # Reduce to 1000 for faster unit testing parity
...
if mse < 1e-10 and disparity < 1e-10 and corr > 0.99999:
    print("\nSUCCESS: DimRedPy UMAP Reduction is mathematically perfect.")
else:
...
plt.suptitle(f"UMAP Reduction Rigorous Verification (N=1000)\n"
             f"Procrustes Disparity: {disparity:.2e} | Pearson R: {corr:.5f} | MSE: {mse:.2e}", 
             fontsize=16, y=1.05, fontweight='bold')

plt.tight_layout()
out_plot = os.path.join(script_dir, "verify_parity_umap_reduction_plot.png")
